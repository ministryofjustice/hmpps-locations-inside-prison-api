...
fun <T, V> KProperty1<T, V>.buildSpecForEqualTo(value: V): Specification<T> = Specification { root, _, criteriaBuilder ->
  criteriaBuilder.equal(root.get<V>(name), value)
}

/** Build «not equal to» specification from an entity’s property */
fun <T, V> KProperty1<T, V>.buildSpecForNotEqualTo(value: V): Specification<T> = Specification { root, _, criteriaBuilder ->
  criteriaBuilder.notEqual(root.get<V>(name), value)
}

/** Build «in» specification from an entity’s property */
fun <T, V> KProperty1<T, V>.buildSpecForIn(values: Collection<V>): Specification<T> = Specification { root, _, criteriaBuilder ->
