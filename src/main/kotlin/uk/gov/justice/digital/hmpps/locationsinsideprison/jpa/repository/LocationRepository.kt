...
interface LocationRepository : JpaRepository<Location, UUID> {
  ...
  @EntityGraph(value = "location.graph", type = EntityGraph.EntityGraphType.LOAD)
  ...
  @Query("update location set residential_housing_type = :residentialHousingType, temporarily_off_cell_cert = (case when location_type = 'CELL' then false end), certified_cell = false, accommodation_type = :accommodationType, location_type_discriminator = (case when location_type = 'CELL' then 'CELL' when code IN ('RECP', 'COURT', 'TAP', 'CSWAP') then 'VIRTUAL' else 'RESIDENTIAL' end) where id = :id", nativeQuery = true)
  @Modifying
  ...
  @Query("update location set location_type = :locationType, certified_cell = false, temporarily_off_cell_cert = (case when residential_housing_type IS NULL then null when :locationType = 'CELL' then false when code IN ('RECP', 'COURT', 'TAP', 'CSWAP') then null end), location_type_discriminator = (case when residential_housing_type IS NULL then 'NON_RESIDENTIAL' when :locationType = 'CELL' then 'CELL' when code IN ('RECP', 'COURT', 'TAP', 'CSWAP') then 'VIRTUAL' else 'RESIDENTIAL' end) where id = :id", nativeQuery = true)
  @Modifying
